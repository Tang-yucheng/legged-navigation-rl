...
// 日志名称
static auto logger_mapping = rclcpp::get_logger("laserMapping");
...
{
    flg_exit = true;
    RCLCPP_WARN(logger_mapping, "catch sig %d", sig);
...
void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
    ...
    double frame_time = stamp.seconds();  // ✅ 返回 double 类型的秒

    if (frame_time < last_timestamp_lidar)
    {
        RCLCPP_ERROR(logger_mapping, "lidar loop back, clear buffer");
    ...
    last_timestamp_lidar = frame_time;

    PointCloudXYZI::Ptr ptr(new PointCloudXYZI());
    PointCloudXYZI::Ptr ptr_div(new PointCloudXYZI());

    double time_div = frame_time;
    ...
    if (cut_frame)
    {
        ...
        for (int i = 0; i < ptr->size(); i++)
        {
            ...
            if (ptr->points[i].curvature / double(1000) + frame_time - time_div > cut_frame_time_interval)
            {
                if (ptr_div->size() < 1)
                    continue;
    ...
    else
    {
        ...
        time_buffer.emplace_back(frame_time);
    }
...
void livox_pcl_cbk(const std::shared_ptr<livox_ros_driver2::msg::CustomMsg> msg)
{
...
void imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr msg_in)
{
    ...
    if (go2_utlidar_imu)
    {
        ...
        msg->orientation = tf2::toMsg(q_inv);

        std::cout << msg->orientation.x << msg->orientation.y << msg->orientation.z << msg->orientation.w << std::endl;
    }

    // 处理时间戳（ROS 2 时间戳处理方式）
    rclcpp::Time original_stamp(msg_in->header.stamp);
    rclcpp::Time corrected_stamp = original_stamp - rclcpp::Duration::from_seconds(time_lag_imu_to_lidar);
    msg->header.stamp = corrected_stamp;

    double timestamp = corrected_stamp.seconds();  // ROS 2 中使用 .seconds()

    mtx_buffer.lock();

    if (timestamp < last_timestamp_imu)
    {
        RCLCPP_ERROR(logger_mapping, "imu loop back, clear deque");
                      ...
                      const std::shared_ptr<rclcpp::Node> &node)
{
    ...
    {
        odomAftMapped.header.stamp = rclcpp::Time(static_cast<uint64_t>(time_current * 1e9));
    }
    else
    {
    ...
    // printf("Odom delay: %f s\n", rclcpp::Clock(RCL_ROS_TIME).now().seconds() - time_current);

    static std::unique_ptr<tf2_ros::TransformBroadcaster> tf_broadcaster = std::make_unique<tf2_ros::TransformBroadcaster>(node);
...
int main(int argc, char **argv)
{
    ...
    path.header.stamp = rclcpp::Time(static_cast<uint64_t>(lidar_end_time * 1e9));
    ...
    {
        sub_pcl = node->create_subscription<livox_ros_driver2::msg::CustomMsg>(lid_topic, 10, livox_pcl_cbk);
    }
    else
    {
        sub_pcl = node->create_subscription<sensor_msgs::msg::PointCloud2>(lid_topic, rclcpp::SensorDataQoS(), standard_pcl_cbk);
    }
    
    // // 创建订阅器
    // auto sub_pcl = node->create_subscription<sensor_msgs::msg::PointCloud2>(lid_topic, rclcpp::SensorDataQoS(), standard_pcl_cbk);

    auto sub_imu = node->create_subscription<sensor_msgs::msg::Imu>(imu_topic, rclcpp::SensorDataQoS(), imu_cbk);

    // 创建发布器
    auto pubLaserCloudFullRes = node->create_publisher<sensor_msgs::msg::PointCloud2>("/pointlio/cloud_registered", rclcpp::QoS(100000));

    auto pubLaserCloudFullRes_body = node->create_publisher<sensor_msgs::msg::PointCloud2>("/pointlio/cloud_registered_body", rclcpp::QoS(100000));

    auto pubLaserCloudEffect = node->create_publisher<sensor_msgs::msg::PointCloud2>("/pointlio/cloud_effected", rclcpp::QoS(100000));

    auto pubLaserCloudMap = node->create_publisher<sensor_msgs::msg::PointCloud2>("/pointlio/laser_map", rclcpp::QoS(100000));

    auto pubOdomAftMapped = node->create_publisher<nav_msgs::msg::Odometry>("/pointlio/odom", rclcpp::QoS(100000));

    auto pubPath = node->create_publisher<nav_msgs::msg::Path>("/pointlio/path", rclcpp::QoS(100000));

    auto plane_pub = node->create_publisher<visualization_msgs::msg::Marker>("/pointlio/planner_normal", rclcpp::QoS(1000));
    ...
    // // 另开线程处理 mapping 主循环（避免堵塞 executor）
    ...
    while (rclcpp::ok())
    {
        ...
        if (flg_reset)
        {
            RCLCPP_WARN(logger_mapping, "reset when rosbag play back");
            p_imu->Reset();
            flg_reset = false;
            continue;
        }
            ...
            for (k = 0; k < time_seq.size(); k++)
            {
                ...
                if (imu_en)
                {
                    ...
                    while (imu_comes)
                    {
                        ...
                        time_predict_last_const = rclcpp::Time(imu_last.header.stamp).seconds();
                ...
                {
                    RCLCPP_WARN(logger_mapping, "No point, skip this scan!\n");
                    idx += time_seq[k];
                    continue;
                }
        ...
        printf("Mapping processing delay: %f s\n", rclcpp::Clock(RCL_ROS_TIME).now().seconds() - Measures.lidar_beg_time);
