...
class CRC(Singleton):
    def __init__(self):
        ...
        # 使用 importlib.resources 打开共享库
        if self.platform == "Linux":
            if platform.machine()=="x86_64":
                with resources.path("legged_control.lib", "crc_amd64.so") as so_path:
                    self.crc_lib = ctypes.CDLL(str(so_path))
            elif platform.machine()=="aarch64":
                with resources.path("legged_control.lib", "crc_aarch64.so") as so_path:
                    self.crc_lib = ctypes.CDLL(str(so_path))
    ...
    def Crc(self, msg):
        ...
        if msg_type == 'unitree_go.msg._low_cmd.LowCmd':
            return self.__Crc32(self.__PackLowCmd(msg))
        elif msg_type == 'unitree_go.msg._low_state.LowState':
            return self.__Crc32(self.__PackLowState(msg))
        if msg_type == 'unitree_hg.msg._low_cmd.LowCmd':
            return self.__Crc32(self.__PackHGLowCmd(msg))
        elif msg_type == 'unitree_hg.msg._low_state.LowState':
            return self.__Crc32(self.__PackHGLowState(msg))
        else:
            raise TypeError(f"unknown ros message type to crc: {msg_type}")
    ...
    def __PackHGLowCmd(self, cmd: HGLowCmd):
        origData = []
    ...
    def __PackHGLowState(self, state: HGLowState):
        origData = []
        origData.extend(state.version)
